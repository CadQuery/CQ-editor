...
def test_debug(main, mocker):
    ...
    r, g, b, a = get_rgba(CQ.child(0).ais[0])
...
def test_render_colors(main_clean):
    ...
    # object 1 (defualt color)
    assert not CQ.child(0).ais[0].HasColor()

    # object 2
    r, g, b, a = get_rgba(CQ.child(1).ais[0])
    assert a == 0.5
    assert r == 1.0
    assert g == 0.0

    # object 3
    r, g, b, a = get_rgba(CQ.child(2).ais[0])
    assert a == 0.5
    assert r == 1.0

    # object 4
    r, g, b, a = get_rgba(CQ.child(3).ais[0])
    assert a == 0.5
    assert r == 1.0

    # object 5
    r, g, b, a = get_rgba(CQ.child(4).ais[0])
    assert a == 0.5
    assert r == 1.0

    # object 6
    r, g, b, a = get_rgba(CQ.child(5).ais[0])
    assert a == 0.5
    assert r == 1.0
...
def test_shading_aspect(main_clean):
    ...
    ma1 = CQ.child(0).ais[0].Attributes().ShadingAspect().Material()
    ma2 = CQ.child(1).ais[0].Attributes().ShadingAspect().Material()
